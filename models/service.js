...
Service.findByCode = function(code, service, done) {
  var where = "ref_code=$1 AND user_id=$2 AND type=$3";
  var v = [code, code.substring(8), service];

  db.where(table, 'id, type, ref_code, options', where, v, 'id', 'ASC', function(err, service) {
...
Service.createSurvey = function(user_id, done) {
  ...
  Service.create({user_id: user_id, type:'Survey', options: options, ref_code: token}, function(err, service) {
    done(err, service);
  });
};
